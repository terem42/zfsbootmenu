...
# By default, ZBM with SSH support waits 30 seconds for a user to connect.
# If no SSH login occurs within that time, boot proceeds automatically.
# This is useful for unattended reboots with a "rescue window" for remote access.
# Set SSH_TIMEOUT to override the default (0 = wait indefinitely).

# ```
# SSH_TIMEOUT=60 ./remote-ssh-build.sh   # Wait 60 seconds
# SSH_TIMEOUT=0 ./remote-ssh-build.sh    # Wait indefinitely
# ```
